...
va_list vap)
{
    extern HSim__s6 * createworkMmultlip(const char*);
    HSim__s6 *blk = createworkMmultlip(label); 
    return blk;
}
...
va_list vap)
{
    extern HSim__s6 * createworkMmult(const char*);
    HSim__s6 *blk = createworkMmult(label); 
    return blk;
}
...
public:
    ...
    HSimConfigDecl * topModuleInstantiate() {
        ...
        (*cfgvh).addVlogModule("work","multlip", (HSimInstFactoryPtr)IF0);
        (*cfgvh).addVlogModule("work","mult", (HSimInstFactoryPtr)IF1);
        (*cfgvh).addVlogModule("work","glbl", (HSimInstFactoryPtr)IF2);
        HSim__s5 * topvl = 0;
        extern HSim__s6 * createworkMmultlip(const char*);
        topvl = (HSim__s5*)createworkMmultlip("multlip");
